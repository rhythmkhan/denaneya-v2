      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch mb-24">
          {plans.map((plan, idx) => (
            <div
              key={idx}
              className={`rounded-3xl p-8 border ${plan.borderClass} flex flex-col justify-between relative transition-all duration-200 hover:shadow-xl`}
            >
              {plan.badge && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-3.5 py-1 rounded-full bg-indigo-600 text-white font-extrabold text-[10px] tracking-wider uppercase shadow-md">
                  {plan.badge}
                </div>
              )}
              ...
              <div className="pt-8 mt-6">
                <a
                  href={plan.ctaLink}
                  className={`w-full py-3.5 rounded-2xl text-xs font-bold flex items-center justify-center gap-2 transition ${
                    plan.isPopular
