...
interface GatewayOption {
  ...
  actionType: 'Send Money' | 'Payment' | 'Cash In';
  color: string;
  accentBg: string;
  accentBorder: string;
}

const DEFAULT_GATEWAYS: GatewayOption[] = [
  {
    id: 'bkash',
    name: 'bKash',
...
export const HostedCheckout: React.FC<HostedCheckoutProps> = ({ invoiceId, initialInvoice }) => {
  ...
  const [activeInstructionTab, setActiveInstructionTab] = useState<'ussd' | 'qr'>('ussd');
  ...
  // Polling & timer
  const [timeLeftSec, setTimeLeftSec] = useState<number>(900); // 15 mins
  ...
  }, [invoiceId, isPaid, isExpired, errorMsg]);

  // 3. Countdown timer tick
  useEffect(() => {
    if (isPaid || isExpired || errorMsg) return;
  ...
  useEffect(() => {
    if (!isPaid || !invoice?.redirect_url) return;
  ...
  const handleVerifyTrx = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isTrxFormatValid) {
      setSubmitFeedback({
        type: 'error',
    ...
    try {
      ...
      if (res.ok && result.success) {
        setIsPaid(true);
        setInvoice((prev: any) => ({ ...prev, ...result, trx_id: trxId.trim() }));
        setSubmitFeedback({
          type: 'success',
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-slate-100">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 text-center space-y-4 border border-slate-200">
          ...
          <h2 className="text-xl font-bold text-slate-900">{errorMsg || 'Invoice Not Found'}</h2>
          <p className="text-xs text-slate-600 leading-relaxed">
            The requested invoice does not exist or the payment link is invalid. Please check your payment link or contact the merchant.
          </p>
          <div className="pt-2">
            <a
              href="/"
              className="inline-flex items-center justify-center px-5 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-medium text-xs transition-colors shadow-sm"
            >
              Back to Home
            </a>
          </div>
        ...
        <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
          ...
          <div className="p-6 space-y-4 text-xs">
            ...
            {invoice?.redirect_url && (
              <div className="p-3 bg-indigo-50 border border-indigo-200 rounded-xl text-indigo-800 text-center font-semibold">
            ...
            <div className="pt-2 flex gap-2">
              <button
                ...
                className="w-full py-3 rounded-xl border border-slate-200 hover:bg-slate-50 font-bold text-slate-700 flex items-center justify-center gap-2 transition"
              >
              ...
              {invoice?.redirect_url && (
                <a
                  ...
                  className="w-full py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-center flex items-center justify-center gap-2 transition"
                >
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-3 sm:p-6 bg-gradient-to-br from-slate-100 via-indigo-50/40 to-slate-200">
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
        ...
        <div className="p-5 sm:p-6 space-y-5">
            ...
            <div className="grid grid-cols-4 gap-2">
              {DEFAULT_GATEWAYS.map(gw => {
                ...
                return (
                  ...
                  >
                    <div 
                      className="w-7 h-7 rounded-lg mx-auto flex items-center justify-center font-bold text-xs text-white shadow-sm mb-1"
          ...
          <div className="flex rounded-xl bg-slate-100 p-1 border border-slate-200">
            <button
              onClick={() => setActiveInstructionTab('ussd')}
              className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition flex items-center justify-center gap-1.5 ${
                activeInstructionTab === 'ussd'
            ...
            <button
              onClick={() => setActiveInstructionTab('qr')}
              className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition flex items-center justify-center gap-1.5 ${
                activeInstructionTab === 'qr'
                  ? 'bg-white text-indigo-700 shadow-sm'
                  : 'text-slate-500 hover:text-slate-800'
              }`}
            >
          ...
          {activeInstructionTab === 'ussd' ? (
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
              ...
              <div className="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                ...
                <button
                  onClick={() => copyToClipboard(selectedGateway.accountNumber, 'acc_no')}
              ...
              <ol className="text-xs text-slate-600 space-y-1.5 pl-4 list-decimal leading-relaxed">
                <li>
                  {t.checkout.step1}{' '}
                  <button
                    onClick={() => copyToClipboard(selectedGateway.ussdCode, 'ussd_str')}
          ...
          <form onSubmit={handleVerifyTrx} className="space-y-3">
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                {trxId.length > 0 && (
                  <span className={`text-[10px] font-bold ${isTrxFormatValid ? 'text-emerald-600' : 'text-amber-600'}`}>
              ...
              <input
                ...
                placeholder={t.checkout.trxIdPlaceholder}
                className="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-sm font-mono tracking-wider uppercase text-slate-900 bg-white"
              />
            ...
            <button
              ...
              className="w-full py-3.5 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:pointer-events-none active:scale-98 text-white text-xs font-bold shadow-lg shadow-indigo-200 transition flex items-center justify-center gap-2"
            >
              {isSubmitting ? (
                <>
