...
exports.ApiDocsViewer = void 0;
const react_1 = __importStar(require("react"));
const LanguageContext_1 = require("./LanguageContext");
const apiDocsData_1 = require("../lib/apiDocsData");
const lucide_react_1 = require("lucide-react");
const ApiDocsViewer = () => {
    const { t, language } = (0, LanguageContext_1.useLanguage)();
    const [selectedEndpointId, setSelectedEndpointId] = (0, react_1.useState)(apiDocsData_1.API_ENDPOINTS[0].id);
    const [selectedLanguage, setSelectedLanguage] = (0, react_1.useState)('curl');
    const [copiedKey, setCopiedKey] = (0, react_1.useState)(null);
    const [baseUrl, setBaseUrl] = (0, react_1.useState)('production');
    const currentEndpoint = apiDocsData_1.API_ENDPOINTS.find(e => e.id === selectedEndpointId) || apiDocsData_1.API_ENDPOINTS[0];
    ...
    return (<div className="py-12 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 text-xs font-bold mb-3 border border-indigo-100">
            <lucide_react_1.Terminal className="w-3.5 h-3.5 text-indigo-600"/>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4 bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="flex gap-2">
              <button onClick={() => setBaseUrl('production')} className={`px-3 py-1.5 rounded-xl text-xs font-mono font-bold transition ${baseUrl === 'production'
            ? 'bg-indigo-600 text-white shadow-sm'
            : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
                https://api.denaneya.com
              </button>
              <button onClick={() => setBaseUrl('sandbox')} className={`px-3 py-1.5 rounded-xl text-xs font-mono font-bold transition ${baseUrl === 'sandbox'
            ? 'bg-indigo-600 text-white shadow-sm'
            : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 bg-white rounded-3xl border border-slate-200 p-4 shadow-sm space-y-2 sticky top-24">
            ...
            {apiDocsData_1.API_ENDPOINTS.map((ep) => {
            const isSelected = ep.id === selectedEndpointId;
            return (<button key={ep.id} onClick={() => setSelectedEndpointId(ep.id)} className={`w-full text-left p-3 rounded-2xl transition flex items-center justify-between ${isSelected
                  ...
                  <lucide_react_1.ChevronRight className={`w-4 h-4 text-slate-400 ${isSelected ? 'text-indigo-600 translate-x-0.5' : ''} transition-transform`}/>
                </button>);
        })}
            ...
            <div className="mt-6 p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
              <div className="flex items-center gap-2 text-xs font-bold text-slate-800">
                <lucide_react_1.Key className="w-3.5 h-3.5 text-indigo-600"/>
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-6">
              ...
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => handleCopy(currentEndpoint.path, 'path')} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition" title="Copy path">
                    {copiedKey === 'path' ? <lucide_react_1.Check className="w-4 h-4 text-emerald-600"/> : <lucide_react_1.Copy className="w-4 h-4"/>}
                  </button>
                </div>
                ...
                <div className="rounded-xl border border-slate-200 overflow-hidden text-xs">
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {currentEndpoint.headers.map((h, i) => (<tr key={i} className="hover:bg-slate-50/50">
              ...
              {currentEndpoint.requestBodySchema && (<div>
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                  ...
                  <div className="rounded-xl border border-slate-200 overflow-hidden text-xs">
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {currentEndpoint.requestBodySchema.map((field, i) => (<tr key={i} className="hover:bg-slate-50/50">
                            ...
                            <td className="px-4 py-2">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${field.required ? 'bg-red-50 text-red-700' : 'bg-slate-100 text-slate-500'}`}>
            ...
            <div className="bg-slate-900 rounded-3xl overflow-hidden shadow-xl border border-slate-800">
              ...
              <div className="bg-slate-950/80 px-4 py-3 border-b border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  {['curl', 'nodejs', 'python', 'php'].map((lang) => (<button key={lang} onClick={() => setSelectedLanguage(lang)} className={`px-3 py-1.5 rounded-xl text-xs font-mono font-bold transition ${selectedLanguage === lang
                ...
                <button onClick={() => handleCopy(currentEndpoint.snippets[selectedLanguage], 'snippet')} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-200 transition">
                  {copiedKey === 'snippet' ? (<>
                      <lucide_react_1.Check className="w-3.5 h-3.5 text-emerald-400"/>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-4">
              ...
              <div className="space-y-4">
                {currentEndpoint.responses.map((resp, i) => (<div key={i} className="rounded-2xl border border-slate-200 overflow-hidden">
                    <div className="bg-slate-50 px-4 py-2.5 border-b border-slate-200 flex items-center justify-between">
                      <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded ${resp.status < 300 ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                        {resp.title}
                      </span>
                      <button onClick={() => handleCopy(JSON.stringify(resp.body, null, 2), `resp_${i}`)} className="text-xs text-slate-400 hover:text-slate-600 flex items-center gap-1">
                        {copiedKey === `resp_${i}` ? <lucide_react_1.Check className="w-3 h-3 text-emerald-600"/> : <lucide_react_1.Copy className="w-3 h-3"/>}
...
exports.ApiDocsViewer = ApiDocsViewer;
//# sourceMappingURL=ApiDocsViewer.js.map