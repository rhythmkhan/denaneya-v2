    ...
    return (<section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {props.map((p, idx) => {
            const Icon = p.icon;
            return (<div key={idx} className="group relative p-7 rounded-3xl bg-slate-50 border border-slate-200/80 hover:bg-white hover:border-indigo-300 hover:shadow-xl hover:shadow-indigo-50 transition-all duration-300 flex flex-col justify-between">
                <div>
                ...
                <div className="pt-5 mt-5 border-t border-slate-200/60 flex items-center justify-between text-xs font-bold text-indigo-600">
                  ...
                  <lucide_react_1.ArrowUpRight className="w-4 h-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform"/>
                </div>
              </div>);
        })}
        </div>
...
//# sourceMappingURL=ValueProps.js.map