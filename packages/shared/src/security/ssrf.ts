 ...
 */
export function isProhibitedIPv4Bytes(a: number, b: number, c: number, _d: number): boolean {
 ...
 */
export function parseIPv6ToBytes(ip: string): Uint8Array | null {
  ...
  if (lastColon !== -1) {
    ...
    if (candidateV4.includes(".")) {
      ...
      for (const p of parts) {
        ...
        if (p.length > 1 && p.startsWith("0")) return null; // Reject octal ambiguity
  ...
  if (doubleColonIdx !== -1) {
    ...
    if (baseStr.indexOf("::", doubleColonIdx + 2) !== -1) return null;
  ...
  if (doubleColonIdx === -1) {
    if (specifiedGroups !== expectedTotalGroups) return null;
  } else {
    if (specifiedGroups > expectedTotalGroups) return null;
  }
 ...
 */
export function isProhibitedIP(ip: string): boolean {
  ...
  if (bytes) {
    ...
    if (bytes[0] === 0x20 && bytes[1] === 0x01 && bytes[2] === 0x00 && bytes[3] === 0x00) {
      if (isProhibitedIPv4Bytes(bytes[4]!, bytes[5]!, bytes[6]!, bytes[7]!)) return true;
      if (
        isProhibitedIPv4Bytes(
          bytes[12]! ^ 0xff,
          bytes[13]! ^ 0xff,
          bytes[14]! ^ 0xff,
          bytes[15]! ^ 0xff
...
export async function validateWebhookUrl(
...
): Promise<{ valid: boolean; pinnedAddress?: string; ipFamily?: number; error?: string }> {
  let parsed: URL;
  try {
  ...
  const isLocalhost =
    cleanHost.toLowerCase() === "localhost" || cleanHost.toLowerCase().endsWith(".localhost");
  ...
  try {
    ...
    return { valid: false, error: `DNS_LOOKUP_ERROR: ${(err as Error).message}` };
  }
}
 ...
 */
export function createPinnedHttpsAgent(pinnedIp: string, ipFamily: number): https.Agent {
