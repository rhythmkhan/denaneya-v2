 ...
 */
const defaultNonceCache = new Map();
 ...
 */
export function clearWebhookNonceCache() {
 ...
 */
export function generateWebhookSignature(payload, secret, explicitTimestamp, explicitNonce) {
 ...
 */
export function verifyWebhookSignature(payload, signatureHeaderOrMap, secret, toleranceSeconds = 300, options = {}) {
    try {
        validateWebhookSecret(secret);
    }
    catch (err) {
    ...
    if (typeof signatureHeaderOrMap === "string") {
        ...
        const candidate = signatureHeaderOrMap["x-denaneya-signature"] ||
